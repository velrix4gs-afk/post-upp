...
export const CoinsDialog = () => {
  ...
  const handlePurchase = async (coins: number, price: number) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('create-coins-checkout', {
        body: {
          coins,
          price
        }
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2 mr-0">
      ...
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {coinPackages.map(pkg => <Card key={pkg.coins} className={`p-4 relative ${pkg.popular ? 'border-primary border-2 shadow-lg' : 'border-border'}`}>
              {pkg.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground px-3 py-1 rounded-full text-xs font-bold">
                  Best Value
                </div>}
              ...
              <div className="text-center space-y-3">
                ...
                <Button className="w-full" onClick={() => handlePurchase(pkg.coins, pkg.price)} disabled={loading}>
